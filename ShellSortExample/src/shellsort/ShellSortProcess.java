...
public class ShellSortProcess {
	...
	public static void byId(ArrayList<Person> personList) {
		...
		sortedPersonArray = shellSort(persons, compTr);
	...
	public static Person[] shellSort(Person[] persons,Comparator comp) {
		
		int h = 1;
	    while((h*3+1) < persons.length)
	        h = 3*h+1;
	    while(h > 0){
	        for(int i = h-1; i < persons.length; i++){
	        	Person s = persons[i];
	            int j = i;
	            for(j = i; (j>=h) && (greaterThan(comp,persons[j-h],s)); j-=h)
	            	persons[j] = persons[j-h];
	            persons[j] = s;
	        }
	        h /= 3;
	    }
